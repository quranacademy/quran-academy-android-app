 ...
 */
class ClearableTextInputEditText @JvmOverloads constructor(
        context: Context,
    ...
    private var clearIconDrawWhenFocused = true
    ...
    private fun init(attrs: AttributeSet?, defStyle: Int) {
        // Load attributes
        val a = context.obtainStyledAttributes(attrs, R.styleable.ClearableTextInputEditText,
                defStyle, 0)

        if (a.hasValue(R.styleable.ClearableTextInputEditText_clearIconDrawable)) {
            clearIconDrawable = a.getDrawable(R.styleable.ClearableTextInputEditText_clearIconDrawable)
    ...
    override fun onSaveInstanceState(): Parcelable? {
        ...
        return if (isClearIconShown)
            ClearIconSavedState(superState, true)
        else
            superState
    }
    ...
    override fun onFocusChanged(focused: Boolean, direction: Int, previouslyFocusedRect: Rect?) {
        showClearIcon((!clearIconDrawWhenFocused || focused) && !TextUtils.isEmpty(text!!.toString()))
