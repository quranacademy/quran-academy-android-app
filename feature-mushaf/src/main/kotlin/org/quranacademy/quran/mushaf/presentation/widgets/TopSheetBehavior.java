package org.quranacademy.quran.mushaf.presentation.widgets;
...
//        mMaxOffset = mParentHeight - peekHeight;
    ...
    private void setStateInternal(@State int state) {
        ...
        if (topSheet != null) {
            for (TopSheetCallback callback : callbacks) {
    ...
    private void dispatchOnSlide(int top) {
        ...
        if (topSheet != null) {
            ...
            float offset;
            if (top < mMinOffset) {
                offset = (float) (top - mMinOffset) / mPeekHeight;
            } else {
                offset = (float) (top - mMinOffset) / ((mMaxOffset - mMinOffset));
            }

            //isExpanding = offset > 0;

            for (TopSheetCallback callback : callbacks) {
                callback.onSlide(topSheet, offset, isOpening);
            }
        }
    }
